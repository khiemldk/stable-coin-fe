...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn('bg-background2 shadow-md p-3', className)}
      classNames={{
        ...
        table: 'w-full border-collapse space-y-1 bg-background2',
        head_row: 'flex',
        head_cell: 'text-muted-foreground rounded-md w-8 font-normal text-[0.8rem]',
        row: 'flex w-full mt-2 rounded-md overflow-hidden',
        cell: 'text-center text-sm p-0 relative [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20',
        day: 'h-8 w-8 p-0 font-normal aria-selected:opacity-100',
        day_selected: '!bg-callToAction-1 hover:bg-main focus:bg-main hover:text-white text-white !rounded-full',
