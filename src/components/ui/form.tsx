...
const FormLabel = React.forwardRef<React.ElementRef<any>, React.ComponentPropsWithoutRef<any>>(
  ({ className, ...props }) => {
    const { formItemId } = useFormField();
    return (
      <label className={cn('mb-1 text-white text-xs font-medium block', className)} htmlFor={formItemId} {...props} />
    );
  }
);
FormLabel.displayName = 'FormLabel';

const FormControl = React.forwardRef<React.ElementRef<any>, React.ComponentPropsWithoutRef<any>>(
  ({ ...props }, ref) => {
...
const FormMessage = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <p ref={ref} id={formMessageId} className={cn('!text-red text-xs font-normal', className)} {...props}>
        {body}
      </p>
    );
  }
